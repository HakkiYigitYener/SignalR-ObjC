...
@interface AFHTTPSessionManager (DumpedMethodFromAFN3)
                             ...
                             failure:(void (^)(NSURLSessionDataTask *, NSError *))failure
{
    ...
    NSURLSessionTask *task = [manager dataTaskWithHTTPMethod:method
                                            ...
                                            downloadProgress:downloadProgress
                                                     success:^(NSURLSessionDataTask *successTask, id responseObject){
        if (success)
        {
            success(successTask, responseObject);
        }
        [operation completeOperation];
    } failure:^(NSURLSessionDataTask *failureTask, NSError *error) {
        if (failure)
        {
            failure(failureTask, error);
        }
        [operation completeOperation];
    }];
